...
#   Exclude selected records from the dataset
...
model = XGBRegressor(learning_rate=0.1, max_depth=9, min_child_weight=24, n_estimators=300, random_state=42)
...
#   Predict the excluded records
X_excluded = X.loc[exclude_indices]
y_excluded_pred = model.predict(X_excluded)

#   Display the actual vs predicted LoS for the excluded records
